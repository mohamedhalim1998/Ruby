...
class Point < GeometryValue
  ...
  # intersect with point is poin if equal
  def intersectPoint p
  ...
  # intersect with line the point if its on the line
  def intersectLine line
  ...
  # shape s and the seg.  seg is an instance of LineSegment
  def intersectWithSegmentAsLineResult seg
    if((((seg.x1 - GeometryExpression::Epsilon) <=  @x && @x <= (seg.x2 + GeometryExpression::Epsilon)) ||
	(((seg.x2 - GeometryExpression::Epsilon) <=  @x) && (@x <= (seg.x1 + GeometryExpression::Epsilon)))) &&
	(((seg.y1 - GeometryExpression::Epsilon) <=  @y && @y <= (seg.y2 + GeometryExpression::Epsilon)) || 
	((seg.y2 - GeometryExpression::Epsilon) <=  @y && @x <= (seg.y1 + GeometryExpression::Epsilon))))
		self
	else
		NoPoints.new
	end
...
class Let < GeometryExpression
  ...
  def eval_prog env 
	if(env)
		if (env.assoc @s)
			@new_env = env
			(@new_env.assoc @s)[1] = @e1.eval_prog(env)
		else
